...
For each meme, make sure you have the right number of text boxes for each template and come up with text for each box. The content in each box should be short and absolutely fit the context of the template.
...
def create_ai_prompt_2():
    ...
    The higher the x value, the more to the right the text box is, and the higher the y value, the closer to the bottom the text box is.
    ...
    for title in memes:
        ...
        for j in range(len(memes[title])):
            ...
            try:
                text += f"x:{meme_dictionary['text_boxes'][j]['x']} y:{1-(meme_dictionary['text_boxes'][j]['y'])}\n"
            except IndexError:
    ...
    for text_box in text_boxes:
            ...
            if meme_dictionary:
                for box in meme_dictionary['text_boxes']:
                    if box['x'] == text_box['x'] and abs((1-box['y']) - text_box['y']) <= 0.01:
...
def wrap_text(text, font, draw, max_width):
    ...
    for word in words:
        test_line = line + word + " "
        textwidth = draw.textlength(test_line, font=font)
...
def draw_text_box(img_path, meme_title, textbox_lst, font_path="impact.ttf"):
    ...
    for textbox in textbox_lst:
        ...
        x, y = textbox['x']*width, textbox['y']*height
        font_size = 100
